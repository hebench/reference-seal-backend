...
public:
    ...
    enum : std::uint64_t
    {
        ...
        Index_NumThreads,
        NumWorkloadParams // This workload requires 3 parameters, and we add 5 encryption params
    };
...
private:
    ...
    std::vector<seal::Plaintext> encodeMatrix(const std::vector<std::vector<double>> &data);
    std::vector<seal::Plaintext> encodeM0(const std::vector<std::vector<double>> &data);
    std::vector<seal::Plaintext> encodeM1(const std::vector<std::vector<double>> &data);
