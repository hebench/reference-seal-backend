...
DotProductBenchmarkDescription::DotProductBenchmarkDescription(hebench::APIBridge::Category category)
{
    ...
    default_workload_params.add<std::uint64_t>(DotProductBenchmarkDescription::DefaultCoeffMudulusBits, "CoefficientMudulusBits");
...
std::string DotProductBenchmarkDescription::getBenchmarkDescription(const hebench::APIBridge::WorkloadParams *p_w_params) const
{
    ...
    std::uint64_t m_poly_modulus_degree                    = bench_params.params[DotProductBenchmarkDescription::Index_PolyModulusDegree].u_param;
    std::uint64_t m_multiplicative_depth                   = bench_params.params[DotProductBenchmarkDescription::Index_NumCoefficientModuli].u_param;
    std::uint64_t m_coeff_mudulus_bits                     = bench_params.params[DotProductBenchmarkDescription::Index_CoefficientModulusBits].u_param;
    std::uint64_t m_scale_bits                             = bench_params.params[DotProductBenchmarkDescription::Index_ScaleExponentBits].u_param;
    if (!s_tmp.empty())
        ss << s_tmp << std::endl;
        ...
        for (uint64_t result_x = 0; result_x < p_param_indexers[1].batch_size; result_x++)
        {
            try
            {
                if (!p_ex)
                {
                    ...
                    result_cipher = m_p_ctx_wrapper->accumulateCKKS(result_cipher, m_vector_size);
                } // end if
            }
            catch (...)
            {
